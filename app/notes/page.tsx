...
import { NotebookPenIcon, MoreVertical, PinIcon, SearchIcon } from "lucide-react"
    ...
    <main className="mx-auto w-full max-w-6xl px-3 py-6 sm:px-4 md:px-6 lg:px-8">
      ...
      <section className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
          ...
          <Button asChild className="w-full sm:w-auto">
            <Link href="/notes/editor">New Note</Link>
          </Button>
        </div>
      </section>
      ...
      {!hasHydrated ? (
        ...
        <Card className="border-dashed">
          ...
          <CardFooter>
            <Button asChild>
              <Link href="/notes/editor">Create Note</Link>
            </Button>
          </CardFooter>
        </Card>
      ) : (
...
function NoteCard({ note }: { note: Note }) {
  ...
  // ✅ Mobile + Desktop Safe PDF Download
  ...
  return (
    <Card
      className="group relative cursor-pointer rounded-xl border bg-card shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg hover:ring-1 hover:ring-primary/20"
      onClick={openEditor}
      tabIndex={0}
      onKeyDown={(e) => (e.key === "Enter" || e.key === " " ? openEditor() : null)}
    >
      {/* Actions */}
      <div
      ...
      >
        ...
        <Button size="sm" variant="secondary" asChild>
          <Link href={`/notes/editor?id=${encodeURIComponent(note.id)}`}>Edit</Link>
        </Button>
      </CardFooter>
    </Card>
  )
